...
namespace BusinessManagement
{
    public partial class PurchaseReport : Form
    {
        ...
        {
            var purchasereport = _db.tblPurchases.Where(p => p.PurchaseDate >= dateTimePicker1.Value && p.PurchaseDate <= dateTimePicker2.Value).Select(x => new { InvoiceNo = x.tblPurchaseInvoice.InvoiceNo, ProductName = x.tblProduct.ProductName, UnitPrice = x.UnitPrice, Quantity = x.Quantity, Total = x.Total, PurchaseDate = x.PurchaseDate, Tax = x.Tax }).ToList();
            dataGridView1.DataSource = purchasereport;
        }
