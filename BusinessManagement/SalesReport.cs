...
namespace BusinessManagement
{
    public partial class SalesReport : Form
    {
        ...
        {
            var salesreport = _db.tblSales.Where(p => p.SalesDate >= dateTimePicker1.Value && p.SalesDate <= dateTimePicker2.Value).Select(x => new { InvoiceNo = x.tblSalesInvoice.InvoiceNo, ProductName = x.tblProduct.ProductName, UnitPrice = x.UnitPrice, Quantity = x.Quantity, Total = x.Total, PurchaseDate = x.SalesDate, Tax = x.Tax }).ToList();
